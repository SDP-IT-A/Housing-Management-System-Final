...
def notice_make():
    ...
    root = tk.Toplevel()
    root.title("Manage Notices")
    root.geometry('600x600')
    root.resizable(False, False)
    image_background = PhotoImage(file=relative_to_assets("notice_make.png"))
    label = tk.Label(root, image=image_background)
    label.place(x=0, y=0)
    entry = tk.Entry(font=("Helvetica", 14), width=40)
    entry.place(x=75, y=200)
    add_image = PhotoImage(file=relative_to_assets("add_button.png"))
    add_button = tk.Button(root, image=add_image, command=add_notice, borderwidth=0, background='lightblue', highlightthickness=0,
                               relief="flat")
    add_button.place(x=200, y=250)
    delete_image = PhotoImage(file=relative_to_assets("delete_button.png"))
    delete_button = tk.Button(root,image=delete_image, command=delete_notice, borderwidth=0, background='lightblue', highlightthickness=0,
                               relief="flat")
    delete_button.place(x=310, y=500)
    refresh_image = PhotoImage(file=relative_to_assets("refresh_button.png"))
    display_button = tk.Button(root, image=refresh_image, command=display_notices, borderwidth=0, background='lightblue', highlightthickness=0,
                               relief="flat")
    display_button.place(x=90, y=500)
    notice_list = tk.Listbox(root, width=75, selectmode=tk.MULTIPLE)
